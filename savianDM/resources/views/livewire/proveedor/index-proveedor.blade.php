            ...
            <div class="flex flex-row justify-between items-center w-full lg:w-auto">
                <div>
                    <div class="flex items-center gap-2 sm:gap-3 mb-1">
                        ...
                        <h3 class="text-xl sm:text-4xl font-black text-slate-800 tracking-tighter">Panel de Proveedores</h3>
                    </div>
                    <p class="hidden sm:block text-slate-400 text-xs sm:text-sm font-medium sm:ml-11">Directorio logístico y de suministros.</p>
                </div>
                ...
                <table class="w-full border-separate border-spacing-y-3">
                    ...
                    <tbody>
                        @foreach ($proveedor as $item)
                            <tr class="group">
                                <td
                                    class="bg-slate-50/50 px-4 sm:px-6 py-4 sm:py-5 rounded-l-2xl sm:rounded-l-3xl border-y border-l border-transparent text-xs font-bold text-slate-400 whitespace-nowrap">
                                    #{{ $item->id }} </td>
                                <td
                                    class="bg-slate-50/50 px-4 sm:px-6 py-4 sm:py-5 border-y border-transparent font-black text-slate-700 whitespace-nowrap">
    ...
    @if ($proveedor)
        ...
        <x-slot name="title">
            <div class="flex items-center gap-4">
                <div
                    class="w-12 h-12 sm:w-16 sm:h-16 rounded-2xl bg-indigo-50 text-indigo-500 flex items-center justify-center shadow-sm">
                ...
                <div>
                    ...
                    <p class="text-[9px] sm:text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Gestión
                        de Suministros</p>
                </div>
            </div>
        </x-slot>
            ...
            <div class="space-y-6 mt-4">
                <div class="space-y-3">
                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Nombre
                        Comercial</label>
                    <input type="text" wire:model="uform.nombre" placeholder="Ej: Distribuidora Central S.A."
                ...
                <div class="bg-slate-50 rounded-2xl p-4 border border-slate-100">
                    <p class="text-[11px] text-slate-500 leading-relaxed">
                        <span class="font-bold text-indigo-500">Nota:</span> Asegúrese de que el nombre coincida con el
                        registro fiscal para evitar duplicados en la facturación.
                    </p>
                </div>
            </div>
        </x-slot>
        ...
        <x-slot name="footer">
            <div class="flex items-center justify-between w-full">
                <button wire:click="cancelar"
                    ...
                    <span
                        class="w-8 h-8 rounded-full bg-white flex items-center justify-center shadow-sm border border-slate-100 group-hover:border-red-100">✕</span>
                    Cancelar
                </button>
