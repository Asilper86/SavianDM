                        ...
                        <tr class="group">
                            ...
                            <td class="bg-slate-50/50 px-6 py-4 rounded-r-3xl border-y border-r border-transparent text-right">
                                <div class="flex justify-end gap-2">
                                    <button wire:click="update({{ $item->id }})" class="p-2 bg-white text-slate-400 hover:text-[#07CBBB] rounded-lg transition-colors border border-slate-50"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" stroke-width="2"/></svg></button>
                                    <button wire:click="lanzarAlerta({{ $item->id }})"  class="p-2 bg-white text-slate-400 hover:text-red-500 rounded-lg transition-colors border border-slate-50">
    ...
    <x-dialog-modal wire:model="openEditar">
        ...
        <x-slot name="footer">
            <div class="flex items-center justify-between w-full">
                <button wire:click="cancelar" class="text-slate-400 hover:text-red-500 font-black text-[10px] uppercase tracking-widest flex items-center gap-2 transition-colors">
                    <span class="w-8 h-8 rounded-full bg-white flex items-center justify-center shadow-sm border border-slate-100">✕</span>
                    Cancelar
                </button>
                
                <button wire:click="editar" wire:loading.attr="disabled" class="bg-[#111827] text-white font-bold px-8 py-4 rounded-2xl shadow-lg flex items-center gap-3 active:scale-95 transition-all text-sm disabled:opacity-50">
