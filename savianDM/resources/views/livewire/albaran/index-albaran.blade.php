        ...
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-4 px-2">
            <div>
                <div class="flex items-center gap-3 mb-1">
                    ...
                    <h3 class="text-2xl sm:text-4xl font-black text-slate-800 tracking-tighter">Registro de Albaranes
                    </h3>
                </div>
                <p class="text-slate-400 text-xs sm:text-sm font-medium sm:ml-11">Control de entradas y salidas de
                    mercancía.</p>
            </div>
            ...
            <table class="w-full border-separate border-spacing-y-3">
                <thead class="sticky top-0 bg-white/10 backdrop-blur-md z-10">
                    <tr class="text-slate-400">
                        <th class="px-6 py-3 text-[10px] font-black uppercase tracking-widest text-left w-24">Nº Albarán
                        </th>
                        <th class="px-6 py-3 text-[10px] font-black uppercase tracking-widest text-left">Fecha de
                            Emisión</th>
                ...
                <tbody>
                    @foreach ($albaran as $item)
                        <tr class="group">
                            <td
                                class="bg-slate-50/50 px-6 py-5 rounded-l-3xl border-y border-l border-transparent text-xs font-black text-slate-500 italic">
                                {{ $item->id }}
                            </td>
                            <td class="bg-slate-50/50 px-6 py-5 border-y border-transparent font-bold text-slate-700">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="w-10 h-10 rounded-xl bg-white flex items-center justify-center shadow-sm border border-slate-100 text-slate-400">
                                        <i class="far fa-calendar-alt"></i>
                                    </div>
                                    <span class="text-sm sm:text-base">{{ $item->created_at }} </span>
                                </div>
                            </td>
                            <td
                                ...
                                <div class="flex justify-end gap-2">
                                    <button title="Descargar PDF"
                                        class="p-3 bg-white text-emerald-500 hover:bg-emerald-500 hover:text-white rounded-xl transition-all border border-slate-100 shadow-sm">
                                        <i class="fas fa-file-pdf"></i>
                                    </button>
                                    <button title="Editar"
                                        class="p-3 bg-white text-blue-500 hover:bg-blue-500 hover:text-white rounded-xl transition-all border border-slate-100 shadow-sm">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button title="Eliminar"
                                        class="p-3 bg-white text-red-500 hover:bg-red-500 hover:text-white rounded-xl transition-all border border-slate-100 shadow-sm">
