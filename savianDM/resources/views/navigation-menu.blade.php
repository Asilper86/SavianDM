<nav x-data="{ open: false }" class="fixed inset-y-0 left-0 w-64 bg-[#07CBBB] dark:bg-gray-900 border-r border-white/10 z-50 flex flex-col transition-all duration-300">
    ...
    <div class="flex-grow overflow-y-auto py-6 px-4 space-y-2">
        ...
        <x-nav-link href="{{ route('dashboard') }}" :active="request()->routeIs('dashboard')" class="flex items-center w-full px-4 py-3 text-white hover:bg-white/10 rounded-xl transition-all border-none">
        ...
        <x-nav-link href="{{ route('moviles') }}" :active="request()->routeIs('moviles')" class="flex items-center w-full px-4 py-3 text-white hover:bg-white/10 rounded-xl transition-all border-none">
            <svg class="w-5 h-5 me-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"></path></svg>
            Moviles
        </x-nav-link>
    ...
    <div class="p-4 border-t border-white/10 bg-black/10">
        <x-dropdown align="right" width="48">
            <x-slot name="trigger">
                <button class="flex items-center w-full text-sm font-medium text-white hover:bg-white/5 p-2 rounded-lg transition">
                    @if (Laravel\Jetstream\Jetstream::managesProfilePhotos())
                        <img class="size-8 rounded-full object-cover me-3 border border-white/20" src="{{ Auth::user()->profile_photo_url }}" alt="{{ Auth::user()->name }}" />
                    @endif
